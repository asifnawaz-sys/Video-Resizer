"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   VIDEO RESIZER by ASIF NAWAZ  --  SIAR DIGITAL  v5.3  (macOS Fixed)        ║
║   Auto MP4 fallback | Perfect smart crop | YOLOv8 | Shopify guard           ║
║   FIXED: Audio bug | macOS bundled ffmpeg | Universal compatibility          ║
╚══════════════════════════════════════════════════════════════════════════════╝

DEPENDENCIES:
    pip install moviepy opencv-python Pillow ttkbootstrap ultralytics imageio-ffmpeg
...
# ---------- logging ----------
LOG_DIR  = Path(os.getcwd()) / "siar_logs"
...
def _find_ffmpeg_exe():
    ...
    Priority:
      1. imageio_ffmpeg bundled binary (works inside .app on macOS)
      2. System PATH
    """
    # 1. Try imageio_ffmpeg (bundled with PyInstaller .app)
    try:
        import imageio_ffmpeg
        bundled = imageio_ffmpeg.get_ffmpeg_exe()
        if bundled and Path(bundled).exists():
            log.info("Using bundled ffmpeg: %s", bundled)
            return bundled
    except Exception as exc:
        log.debug("imageio_ffmpeg not available: %s", exc)
    ...
    for candidate in ["ffmpeg", "ffmpeg.exe"]:
        try:
            ...
            if r.returncode == 0:
                log.info("Using system ffmpeg: %s", candidate)
                return candidate
        except Exception:
            pass
...
if HAS_FFMPEG:
    try:
        r = subprocess.run([FFMPEG_EXE, "-version"],
                           capture_output=True, text=True, timeout=10)
        FFMPEG_VERSION = r.stdout.splitlines()[0] if r.stdout else "unknown"
        log.info("ffmpeg OK: %s", FFMPEG_VERSION)
    except Exception as exc:
...
def get_available_encoders():
    ...
    if not HAS_FFMPEG:
        return encoders
    try:
        r = subprocess.run([FFMPEG_EXE, "-encoders"],
                           capture_output=True, text=True, timeout=10)
        if r.returncode == 0:
...
def yolo_detect(frame_bgr, tw, th):
    try:
        ...
        for i, (box, conf, cid) in enumerate(zip(boxes, confs, cls_ids)):
            ...
            dets.append({"class": name, "conf": float(conf),
                         "bbox": tuple(int(v) for v in box), "color": color})
        ...
        if sr > tr:
            cw2 = int(h * tr)
            x1  = int(cx - cw2 / 2)
            x1  = max(0, min(x1, w - cw2))
            crop = (x1, 0, x1 + cw2, h)
        else:
            ch2 = int(w / tr)
            y1  = int(cy - ch2 / 2)
            y1  = max(0, min(y1, h - ch2))
            crop = (0, y1, w, y1 + ch2)
        return crop, dets
    except Exception as exc:
...
def _get_video_info(path):
    ...
    try:
        ...
        jf = _ffprobe_json(path, select_streams="",
...
def build_vf(placement, w_in, h_in, tw, th, crop_bbox=None):
    ...
    if tw_even != tw or th_even != th:
        log.warning(f"Target {tw}x{th} not even → using {tw_even}x{th_even}")
    try:
        def _cover_crop():
    ...
    try:
        ...
        if r.returncode != 0:
            log.warning("%s failed (rc=%d):\n%s",
                        label, r.returncode, r.stderr[-1200:])
...
def _build_ffmpeg_cmd(in_path, out_path, vf, is_complex,
                      vcodec, acodec, fmt, crf, speed,
                      fps_opt, audio_br, include_audio,
                      extra_v_flags=None):
    """
    Build a complete ffmpeg command list.
    include_audio=True  → encode audio stream
    include_audio=False → -an  (no audio, silent output intentional)
    extra_v_flags       → list of additional video flags (e.g. ['-preset','ultrafast'])
    """
...
def encode_file(in_path, out_path, cfg, vf, vi, log_cb):
    ...
    # ------------------------------------------------------------------
    # Helper: try one encode attempt
    # ------------------------------------------------------------------
    def _try(vcodec_, acodec_, fmt_, out_, include_audio_, label,
             extra_v=None):
        cmd = _build_ffmpeg_cmd(
            in_path, out_, vf, is_complex,
            vcodec_, acodec_, fmt_, crf, speed,
            fps_opt, audio_br, include_audio_,
    ...
    # ------------------------------------------------------------------
    # ATTEMPT 1 – primary format with audio (if desired)
    # ------------------------------------------------------------------
    log_cb(f"   CRF={crf}  speed={speed}  format={fmt}", "info")
    ok, stderr = _try(codec, acodec, fmt, out_path, want_audio, "primary")

    # ------------------------------------------------------------------
    # ATTEMPT 2 – if audio encoder missing, retry WITHOUT audio
    #             (only when the video itself succeeded partially)
    # ------------------------------------------------------------------
    if not ok and want_audio:
        audio_err = any(kw in stderr.lower()
                        for kw in ["encoder", "aac", "opus", "audio"])
        if audio_err:
            log_cb("   Audio encoder issue – retrying without audio", "warn")
            ok, stderr = _try(codec, acodec, fmt, out_path,
                              False, "primary-no-audio")

    # ------------------------------------------------------------------
    # ATTEMPT 3 – WEBM failed → fallback to MP4  (keep audio if possible)
    # ------------------------------------------------------------------
    if not ok and fmt == "WEBM":
        log_cb("   WEBM failed – falling back to MP4", "warn")
        fb_codec, fb_acodec, fb_ext = CODEC_MAP["MP4"]
        fb_out = out_path.with_suffix(fb_ext)

        ok, stderr = _try(fb_codec, fb_acodec, "MP4", fb_out,
                          want_audio, "fallback-MP4")
        if not ok and want_audio:
            log_cb("   MP4 fallback: audio failed – retrying without audio", "warn")
            ok, stderr = _try(fb_codec, fb_acodec, "MP4", fb_out,
                              False, "fallback-MP4-no-audio")
        if ok:
            out_path = fb_out
            fmt      = "MP4"
            codec    = fb_codec
    ...
    # ------------------------------------------------------------------
    # ATTEMPT 4 – ultrafast preset as last resort
    # ------------------------------------------------------------------
    if not ok:
        log_cb("   Retrying with ultrafast preset...", "warn")
        if stderr:
            for line in stderr.strip().splitlines()[-6:]:
                if line.strip():
                    log_cb(f"   ffmpeg: {line.strip()}", "error")

        ok, stderr = _try(codec, acodec, fmt, out_path,
                          want_audio, "ultrafast",
                          extra_v=["-preset", "ultrafast"])
        if not ok and want_audio:
            log_cb("   ultrafast: audio failed – retrying without audio", "warn")
            ok, stderr = _try(codec, acodec, fmt, out_path,
                              False, "ultrafast-no-audio",
                              extra_v=["-preset", "ultrafast"])
    ...
    if max_bytes < 999 * 1024 * 1024 and actual > max_bytes:
        ...
        log_cb(f"   Over limit – 2-pass size-guard "
               f"({actual_mb:.2f} MB → target {vid_kbps} kbps)", "warn")

        null_out = "/dev/null" if sys.platform != "win32" else "NUL"
        fmt_flag = "webm" if fmt == "WEBM" else "mp4"

        def _bitrate_cmd(pass_num, out_target):
            cmd = [FFMPEG_EXE, "-y", "-i", str(in_path)]
            if fps_opt != "Source FPS":
                cmd += ["-r", fps_opt]
            if is_complex:
                ...
                cmd += ["-vf", vf]
            cmd += ["-c:v", codec, "-b:v", f"{vid_kbps}k"]
            if fmt == "WEBM":
            ...
            if pass_num == 1:
                cmd += ["-f", fmt_flag, null_out]
            else:
                cmd += ["-pix_fmt", "yuv420p", "-movflags", "+faststart"]
                # Audio in pass 2 only
                if want_audio:
                    cmd += ["-c:a", acodec, "-b:a", audio_br,
                            "-ar", "44100", "-ac", "2"]
                else:
                    cmd += ["-an"]
                cmd += [str(out_target)]
        ...
        ok1, _ = _run_ffmpeg(_bitrate_cmd(1, None), "2pass-1st")
        if ok1:
            ok2, _ = _run_ffmpeg(_bitrate_cmd(2, out_path), "2pass-2nd")
            if ok2:
                try:
                    actual = out_path.stat().st_size
                except:
                ...
                log_cb("   2-pass 2nd failed – keeping CRF output", "warn")
        else:
            log_cb("   2-pass 1st failed – keeping CRF output", "warn")

        # cleanup 2-pass log files
        for f in out_path.parent.glob("ffmpeg2pass-0*"):
            try:
                f.unlink()
            except:
                pass
...
class VideoProcessor:
    ...
    def process_batch(self, file_list, cfg):
        ...
        self._log(
            f"BATCH COMPLETE  --  {done} OK  |  {failed} FAILED  |  {total} TOTAL",
            "head")
    ...
    def _process_one(self, in_path, out_path, cfg):
        ...
        self._log(
            f"   Probe: {vi['width']}x{vi['height']}  "
            f"{vi['fps']:.2f}fps  {vi['duration']:.1f}s  "
            f"audio={'yes' if vi['has_audio'] else 'no'}", "info")
            ...
            if HAS_YOLO and HAS_CV2:
                ...
                if frame is not None:
                    ...
                    if detections:
                        ...
                        try:
                            self.detection_cb(frame, detections)
                        except:
                            pass
                    else:
        ...
        self._log(
            f"   Filter: {vf[:120]}{'...' if len(vf)>120 else ''}", "info")
        return encode_file(str(in_path), out_path, cfg, vf, vi, self._log)
...
class BarMeter(tk.Canvas):
    def __init__(self, parent, color=P.G1, height=5, **kw):
        super().__init__(parent, height=height, bg=P.BG0,
    ...
    def _draw(self):
        try:
            ...
            if filled > 0:
                self.create_rectangle(0,0,filled,h,
...
class DetectionPanel(tk.Frame):
    ...
    def _build(self):
        hdr = tk.Frame(self, bg=P.BG2)
        hdr.pack(fill="x", padx=8, pady=(6,3))
        tk.Label(hdr, text="YOLOv8  DETECTION  MONITOR",
                 font=P.MONO_B, fg=P.PU, bg=P.BG2).pack(side="left")
        ...
        self.img_cv = tk.Canvas(self, bg=P.BG0, height=190,
                                highlightthickness=1,
                                highlightbackground=P.BDR)
    ...
    def _no_det_text(self):
        try:
            ...
            self.img_cv.create_text(cw//2, 95, text="No detection yet",
                                    fill=P.TXT3, font=P.MONO_S)
        except:
            pass
    ...
    def update_detection(self, frame_bgr, detections):
        ...
        try:
            ...
            self.img_cv.create_image(cw//2, img.height//2,
...
class VideoResizerApp:
    _LOGO_FRAMES = [P.G1,"#00ee77","#00dd66",P.G1,
                    P.B1,"#00bbdd",P.B1,P.G1]
    _logo_idx = 0

    def __init__(self, root):
        self.root = root
        self.root.title("Video Resizer by Asif Nawaz  |  SIAR DIGITAL")
    ...
    def _apply_style(self):
        try:
            ...
            style.configure("TCombobox",
                            fieldbackground=P.BG4, background=P.BG3,
                            foreground=P.TXT, selectbackground=P.PU2,
                            selectforeground="#fff", bordercolor=P.BDR2,
                            arrowcolor=P.G1)
            style.map("TCombobox",
                      fieldbackground=[("readonly", P.BG4)],
                      foreground=[("readonly", P.TXT)])
            style.configure("Green.Horizontal.TProgressbar",
                            troughcolor=P.BG0, background=P.G1,
                            bordercolor=P.BG0, lightcolor=P.G1,
                            darkcolor=P.G2)
        except:
            pass
    ...
    def _build_ui(self):
        self._build_topbar()
        body = tk.Frame(self.root, bg=P.BG1)
        body.pack(fill="both", expand=True)
        left_wrap = tk.Frame(body, bg=P.BG1)
        left_wrap.pack(side="left", fill="both", expand=True)
        self._build_scrollable_left(left_wrap)
        right = tk.Frame(body, bg=P.BG0, width=400)
    ...
    def _build_topbar(self):
        bar = tk.Frame(self.root, bg=P.BG0, height=56)
        bar.pack(fill="x")
        bar.pack_propagate(False)
        left = tk.Frame(bar, bg=P.BG0)
        left.pack(side="left", fill="y", padx=(10,0))
        self.logo_lbl = tk.Label(left, text="🎬  VIDEO RESIZER  🎬",
                                 font=P.MONO_H, fg=P.G1, bg=P.BG0)
        ...
        tags = [
            ...
            ("Smart Crop",P.G1,True),  ("Mirror BG",P.CY,True),
            ("Shopify Guard",P.OR,True),("Folder Mirror",P.YL,True),
            ("CRF+2pass",P.PK,True),   ("ffmpeg",P.G1,HAS_FFMPEG),
        ]
        for tag, col, ok in tags:
            c = col if ok else P.TXT3
            f = tk.Frame(left, bg=P.BG3, highlightthickness=1,
    ...
    def _build_scrollable_left(self, parent):
        ...
        col_b = tk.Frame(self.sf, bg=P.BG1)
        col_b.pack(side="left", fill="both", expand=True, padx=(3,6), pady=4)
    ...
    def _card(self, parent, title, accent=P.G1):
        outer = tk.Frame(parent, bg=P.BG1)
        outer.pack(fill="x", pady=(6,0))
        hdr = tk.Frame(outer, bg=P.BG2)
        hdr.pack(fill="x")
        tk.Frame(hdr, bg=accent, width=4).pack(side="left", fill="y")
        tk.Label(hdr, text=f"  {title}  ",
                 font=P.MONO_B, fg=accent, bg=P.BG2, pady=6).pack(side="left")
        body = tk.Frame(outer, bg=P.BG3,
                        highlightthickness=1, highlightbackground=P.BDR)
        body.pack(fill="x")
        return body
    ...
    def _build_input_panel(self, p):
        ...
        self.file_count_lbl = tk.Label(body, text="No files queued",
                                       font=P.MONO_S, fg=P.TXT3, bg=P.BG3)
        self.file_count_lbl.pack(anchor="w", padx=12, pady=(0,4))
        lbf = tk.Frame(body, bg=P.BG0,
                       highlightthickness=1, highlightbackground=P.BDR)
        lbf.pack(fill="x", padx=10, pady=(0,2))
    ...
    def _build_size_panel(self, p):
        ...
        cb = ttk.Combobox(body, textvariable=self.preset_var,
                          values=list(PRESETS.keys()),
                          state="readonly", font=P.MONO)
        cb.pack(fill="x", padx=10, pady=(10,6))
        cb.bind("<<ComboboxSelected>>", self._on_preset_change)
        dr = tk.Frame(body, bg=P.BG3)
        dr.pack(fill="x", padx=10, pady=(0,10))
        for lbl, var, col in [
            ("WIDTH  px", self.width_var,  P.B1),
            ("HEIGHT px", self.height_var, P.PK),
        ]:
            cf = tk.Frame(dr, bg=P.BG3)
    ...
    def _build_category_panel(self, p):
        ...
        row  = tk.Frame(body, bg=P.BG3)
        row.pack(fill="x", padx=10, pady=8)
        for name, color in CATEGORY_MODES:
            f = tk.Frame(row, bg=P.BG4, cursor="hand2",
                         highlightthickness=1, highlightbackground=P.BDR)
    ...
    def _build_format_panel(self, p):
        ...
        fr = tk.Frame(body, bg=P.BG3)
        fr.pack(fill="x", padx=10, pady=(10,4))
        tk.Label(fr, text="FORMAT:", font=P.MONO_S,
                 fg=P.TXT3, bg=P.BG3).pack(side="left", padx=(0,8))
        ...
        tk.Label(qr, text="QUALITY:", font=P.MONO_S,
                 fg=P.TXT3, bg=P.BG3).pack(side="left", padx=(0,8))
        ttk.Combobox(qr, textvariable=self.quality_var,
        ...
        for lbl, var, opts, w in [
            ("AUDIO:", self.audio_var, AUDIO_BITRATES, 10),
            ("FPS:",   self.fps_var,   FPS_OPTIONS,    13),
        ]:
            tk.Label(af, text=lbl, font=P.MONO_S,
                     fg=P.TXT3, bg=P.BG3).pack(side="left", padx=(0,4))
            ttk.Combobox(af, textvariable=var, values=opts,
                         state="readonly", font=P.MONO,
                         width=w).pack(side="left", padx=(0,14))

    def _build_advanced_panel(self, p):
        body = self._card(p, "ADVANCED SETTINGS", P.TXT2)
        row  = tk.Frame(body, bg=P.BG3)
        row.pack(fill="x", padx=10, pady=10)
        tk.Label(row, text="ENCODE SPEED:", font=P.MONO_S,
                 fg=P.TXT3, bg=P.BG3).pack(side="left", padx=(0,6))
        ttk.Combobox(row, textvariable=self.speed_var, values=SPEED_PRESETS,
                     state="readonly", font=P.MONO,
                     width=12).pack(side="left", padx=(0,14))
        for txt, var, col in [
            ("Auto ZIP",  self.autozip_var,   P.G1),
            ("Skip Done", self.skip_done_var, P.B1),
        ]:
            tk.Checkbutton(row, text=txt, variable=var,
                           font=P.MONO_S, fg=col, bg=P.BG3,
                           selectcolor=P.BG3, activebackground=P.BG3,
                           activeforeground=col).pack(side="left", padx=(0,12))
    ...
    def _build_placement_panel(self, p):
        ...
        for label, key, is_ai in PLACEMENT_MODES:
            ...
            inner = tk.Frame(f, bg=P.BG3)
            inner.pack(fill="x", padx=4, pady=4)
            rb = tk.Radiobutton(inner, variable=self.placement_var, value=key,
                                bg=P.BG3, activebackground=P.BG3,
                                selectcolor=P.PK, fg=P.PK,
                                command=lambda k=key, fr=f:
                                    self._sel_placement(k, fr))
            rb.pack(side="left")
            tk.Label(inner, text=f"  {label}",
                     font=P.MONO, fg=P.TXT, bg=P.BG3).pack(side="left")
            if is_ai:
            ...
            for widget in ([f, inner] + list(inner.winfo_children())):
                try:
                    widget.bind("<Button-1>",
                                lambda e, k=key, fr=f: (
                                    self.placement_var.set(k),
                                    self._sel_placement(k, fr)))
                except:
                    pass
        self._sel_placement("smart_crop",
                            self._placement_frames["smart_crop"])
            ...
            for k, f in self._placement_frames.items():
                ...
                for w in f.winfo_children():
                    ...
                    for ww in (w.winfo_children()
                               if hasattr(w,"winfo_children") else []):
    ...
    def _build_shopify_panel(self, p):
        ...
        tk.Label(row, text="  Max:", font=P.MONO_S,
                 fg=P.TXT3, bg=P.BG3).pack(side="left")
        ttk.Combobox(row, textvariable=self.max_mb_var,
                     values=["5","7","10","15","20","50","100"],
                     state="readonly", font=P.MONO,
                     width=5).pack(side="left")
        tk.Label(row, text=" MB", font=P.MONO_S,
                 fg=P.TXT, bg=P.BG3).pack(side="left")
        tk.Label(body,
    ...
    def _build_output_panel(self, p):
        body = self._card(p,
                          "OUTPUT FOLDER  (subfolder structure mirrored)",
                          P.G1)
        row = tk.Frame(body, bg=P.BG3)
        row.pack(fill="x", padx=10, pady=10)
        tk.Entry(row, textvariable=self.output_dir,
                 font=P.MONO, fg=P.TXT, bg=P.BG0,
                 insertbackground=P.G1, relief="flat",
                 highlightthickness=1,
                 highlightbackground=P.BDR).pack(
                     side="left", fill="x", expand=True, ipady=5)
        NeonBtn(row, " Browse ", self._browse_output,
                P.G1).pack(side="right", padx=(4,0))
        NeonBtn(row, " Open ",   self._open_output,
                P.B1).pack(side="right", padx=(4,0))

    def _build_right_panel(self, p):
        tk.Label(p, text="  LIVE PREVIEW",
                 font=P.MONO_B, fg=P.G1, bg=P.BG0
                 ).pack(anchor="w", pady=(8,3))
        self.preview_cv = tk.Canvas(p, bg=P.BG0, height=180,
                                    highlightthickness=1,
                                    highlightbackground=P.BDR)
        ...
        self.det_panel = DetectionPanel(p)
        self.det_panel.pack(fill="x")
        tk.Frame(p, bg=P.BDR, height=1).pack(fill="x", padx=8, pady=(2,4))
        tk.Label(p, text="  STATISTICS",
                 font=P.MONO_B, fg=P.G1, bg=P.BG0).pack(anchor="w")
        sg = tk.Frame(p, bg=P.BG0)
        ...
        tk.Label(p, text="  PROGRESS",
                 font=P.MONO_B, fg=P.G1, bg=P.BG0
                 ).pack(anchor="w", pady=(4,2))
        self.progress_bar = ttk.Progressbar(
            p, mode="determinate", maximum=100,
            style="Green.Horizontal.TProgressbar")
        self.progress_bar.pack(fill="x", padx=8, pady=(0,2))
        self.progress_txt = tk.Label(p, text="0 / 0  (0.0%)",
                                     font=P.MONO_S, fg=P.TXT3, bg=P.BG0)
        self.progress_txt.pack(anchor="w", padx=10)

        tk.Frame(p, bg=P.BDR, height=1).pack(fill="x", padx=8, pady=(6,2))
        lhdr = tk.Frame(p, bg=P.BG0)
        lhdr.pack(fill="x", padx=8, pady=(2,2))
        tk.Label(lhdr, text="SESSION LOG",
        ...
        for tag, col in [
            ("ok",P.G1),("error",P.RD),("warn",P.OR),
            ("yolo",P.PU),("info",P.B1),("head",P.YL),
        ]:
            self.log_box.tag_config(tag, foreground=col)

        tk.Frame(p, bg=P.BDR, height=1).pack(fill="x", padx=8, pady=(2,4))
        tk.Label(p, text="  AI LIBRARY STATUS",
                 font=P.MONO_B, fg=P.G1, bg=P.BG0).pack(anchor="w")
        libs = [
            ("YOLOv8",  HAS_YOLO,    P.PU, "pip install ultralytics"),
            ("OpenCV",  HAS_CV2,     P.B1, "pip install opencv-python"),
            ("moviepy", HAS_MOVIEPY, P.G1, "pip install moviepy"),
            ("Pillow",  HAS_PIL,     P.YL, "pip install Pillow"),
            ("ttk",     HAS_TB,      P.PK, "pip install ttkbootstrap"),
            ("ffmpeg",  HAS_FFMPEG,  P.CY, "pip install imageio-ffmpeg"),
        ]
        for name, ok, col, hint in libs:
            bg2 = "#041408" if ok else "#140408"
            f = tk.Frame(p, bg=bg2,
                         highlightthickness=1,
                         highlightbackground=col if ok else P.RD)
    ...
    def _build_bottombar(self):
        ...
        tk.Button(bar, text="  STOP  ",
                  ...
                  ).pack(side="right", fill="y", padx=(0,10), pady=8)
        self.status_lbl = tk.Label(bar, text="Ready.",
                                   font=P.MONO, fg=P.TXT3, bg=P.BG0)
    ...
    def _preview_placeholder(self):
        try:
            ...
            self.preview_cv.create_text(200, 90, text="No Preview",
                                        fill=P.TXT3, font=P.MONO_S)
        except:
            pass
                ...
                for f in files:
                    if f not in seen:
                        self.input_files.append(
                            {"abs": f, "rel": os.path.basename(f)})
                self._refresh_list()
        except Exception as exc:
            log.error("_add_files: %s", exc)

    def _add_folder(self):
        try:
            folder = filedialog.askdirectory(
                title="Select Root Folder (all subfolders scanned)")
            if not folder:
                return
            ...
            seen = {i["abs"] for i in self.input_files}
            for item in found:
                if item["abs"] not in seen:
                    self.input_files.append(item)
            self._refresh_list()
            subdirs = {str(Path(i["rel"]).parent)
                       for i in self.input_files
                       if str(Path(i["rel"]).parent) != "."}
            if subdirs:
                self.struct_lbl.config(
                    text=f"  {len(subdirs)} subfolder(s) found – structure mirrored")
            else:
                self.struct_lbl.config(text="  Flat folder – no subfolders")
        except Exception as exc:
            log.error("_add_folder: %s", exc)

    def _clear_files(self):
        self.input_files.clear()
        self.struct_lbl.config(text="")
        self._refresh_list()
    ...
    def _refresh_list(self):
        try:
            ...
            for item in self.input_files:
                self.file_lb.insert("end", f"  {item['rel']}")
            n = len(self.input_files)
            self.file_count_lbl.config(
                text=f"  {n} video{'s' if n!=1 else ''} queued",
                fg=P.G1 if n>0 else P.TXT3)
        except:
            pass
    ...
    def _start(self):
        ...
        if not self.input_files:
            messagebox.showwarning("No Files",
                                   "Add video files or a folder first.")
            return
        try:
            ...
            messagebox.showerror("Size Error",
                                 "Enter valid Width and Height.")
            return
        if self.shopify_var.get():
            try:
                max_bytes = int(float(self.max_mb_var.get()) * 1024 * 1024)
            except:
                max_bytes = 7 * 1024 * 1024
        else:
            max_bytes = 999 * 1024 * 1024
        ...
        cfg = {
            ...
            "crf":          QUALITY_PRESETS.get(self.quality_var.get(), "22"),
            "speed":        self.speed_var.get(),
            "fps":          self.fps_var.get(),
            "audio_bitrate":self.audio_var.get(),
            "output_dir":   self.output_dir.get(),
            "auto_zip":     self.autozip_var.get(),
            "skip_done":    self.skip_done_var.get(),
            "max_bytes":    max_bytes,
        }
        self._clear_log()
        self.det_panel.clear()
        ...
        self.processor = VideoProcessor(
            ...
            detection_cb=self._on_detection)
        self._proc_thread = threading.Thread(
            target=self._run_batch,
            args=(list(self.input_files), cfg),
            daemon=True)
        self._proc_thread.start()

    def _run_batch(self, files, cfg):
        try:
            self.processor.process_batch(files, cfg)
        except Exception as exc:
            log.error("_run_batch: %s", traceback.format_exc())
    ...
    def _on_done(self):
        try:
            ...
            self.status_lbl.config(text="Done!", fg=P.G1)
        except:
            pass
    ...
    def _on_preview(self, video_path):
        def _do():
            ...
            try:
                ...
                self.preview_cv.create_image(cw//2, img.height//2,
                                             anchor="center",
                                             image=imgtk)
                ...
                self.preview_name.config(
                    text=f"  {Path(video_path).name}  --  {sz:.2f} MB",
                    fg=col)
            except Exception as exc:
                log.debug("_on_preview error: %s", exc)
        try:
            self.root.after(0, _do)
        except:
            pass
    ...
    def _on_detection(self, frame_bgr, detections):
        def _do():
            try:
                self.det_panel.update_detection(frame_bgr, detections)
            except:
                pass
        try:
            self.root.after(0, _do)
